...
const ANCHOR_INTL =
  /\b(couramment|ma[ií]tris\w*|exig[ée]\w*|requis\w*|obligatoire|souhait\w*|appr[ée]ci\w*|atout|maternel\w*|bilingue|trilingue|niveau|connaissance\w*|\w*kenntniss\w*|flie[sß]end|verhandlungssicher|erforderlich|zwingend|voraussetzung|muttersprach\w*|w[üu]nschenswert|vorteil\w*|vloeiend|vereist|verplicht|moedertaal|voordeel\w*|talenkennis\w*|fluido|nativo|requerid\w*|imprescindible|obligatori\w*|valorable|deseable|conocimiento\w*|fluente|madrelingua|richiest\w*|obbligatori\w*|gradito|conoscenza)/i;
...
function levelSignals(sentence) {
  ...
  scan(/\b(exig[ée]e?s?|requis\w*|obligatoire|ma[ií]tris\w*|couramment|maternel\w*|bilingue|trilingue|erforderlich|zwingend|voraussetzung|flie[sß]end|verhandlungssicher|muttersprach\w*|vereist|verplicht|vloeiend|moedertaal|requerid\w*|imprescindible|obligatori\w*|fluid[oa]\w*|nativ[oa]\w*|richiest\w*|obbligatori\w*|fluente|madrelingua)\b/gi, 'required');
...
export function selfTest() {
  ...
  // 11) English words that LOOK like FR/ES stems must NOT fabricate a requirement
  {
    const exigent = find(extractLanguages('The exigencies of this English role are many.'), 'English');
    ok(!exigent || exigent.level !== 'required', 'English "exigencies" → not a fabricated requirement');
    const fluid = find(extractLanguages('English language tasks need fluid handovers.'), 'English');
    ok(!fluid || fluid.level !== 'required', 'English "fluid" → not a fabricated requirement');
    ok(!find(extractLanguages('Keeping English documentation current and courant.'), 'English') ||
       find(extractLanguages('Keeping English documentation current and courant.'), 'English').level !== 'required',
       'English "courant" → not a fabricated requirement');
    ok(!find(extractLanguages('Configure the English dominio settings.'), 'English') ||
       find(extractLanguages('Configure the English dominio settings.'), 'English').level !== 'required',
       'English "dominio" → not a fabricated requirement');
    // the genuine FR forms still work after tightening
    eq(find(extractLanguages('Maîtrise du français exigée.'), 'French')?.level, 'required', 'FR "exigée" still → required');
  }
