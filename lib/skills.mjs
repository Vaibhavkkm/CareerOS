...
// Indeed / markdown escape backslashes before ASCII punctuation: "8\+ years",
// "service\-oriented", "C\+\+". Strip them so parsing sees the real text — this
// was the bug that hid "8+ years" from the experience parser.
export const deEscape = (s) => String(s || '').replace(/\\([-+&%$#_.~(){}\[\]/])/g, '$1');

// ─── core recognizer ──────────────────────────────────────────────────────────
// Returns { hits:[{canonical, alias, index, li, line, ambiguous, meta}], lines }.
export function recognize(text) {
  const src = deEscape(text);
...
// A people-management / senior-leadership role — surfaced so the scorer can damp it
// hard for a candidate with no management background (a junior IC ≠ a DS Manager).
// Leadership ROLE nouns — reliable ONLY in the title (in the body they're noise:
// "report to the manager", "work with product managers", "hiring managers"…).
const LEAD_ROLE = /\b(managers?|head of|directors?|vps?|vice president|chief|cto|cdo|ceo|managing director|people lead|team lead|engineering manager)\b/i;
// Management DUTY phrases — safe to detect in the body (they describe the job, not a
// passing mention of some other manager).
const MGMT_DUTY = /\b(people management|line management|direct reports|manag(?:e|ing)[a-z]*\s+(?:a\s+|the\s+|your\s+)?team|lead(?:ing)?\s+(?:and\s+managing\s+)?(?:a\s+|the\s+|your\s+|large\s+)?teams?|leadership\s+of\s+(?:a\s+|the\s+|large\s+)?teams?|build\s+and\s+lead\s+(?:a\s+)?team|hire\s+and\s+(?:develop|manage))\b/i;
// Don't fire on a NEGATED ("no people management") or REFERENCED ("report to the
// Director", "work with managers") mention — only on the role's own duties.
const NEG_OR_REF = /\b(no|not|without|never|reports?\s+to|reporting\s+to|report\s+to|work(?:ing)?\s+with|alongside|such as|e\.g\.?|board of)\b/i;
export function isManagerialJd(title, text) {
  if (LEAD_ROLE.test(String(title || ''))) return true; // the title is the reliable signal
  const body = deEscape(text);
  const g = new RegExp(MGMT_DUTY.source, 'gi');
  let m;
  while ((m = g.exec(body))) {
    if (!NEG_OR_REF.test(body.slice(Math.max(0, m.index - 28), m.index))) return true;
  }
  return false;
}

// ─── target-role / job-function fit ───────────────────────────────────────────
// A role can share the candidate's TOOLS yet be a different FUNCTION (a "Reliability
// Engineer" using Python/SQL/ML is not a Data Scientist). We compare the JD's role
// title to the candidate's target roles by their DISTINCTIVE tokens — dropping
// generic role-suffix words ("engineer" is shared by data engineer AND reliability
// engineer; the signal is "data"/"ml" vs "reliability").
const GENERIC_ROLE = new Set([
  'engineer', 'engineering', 'developer', 'dev', 'programmer', 'senior', 'junior', 'jr', 'sr',
  'lead', 'principal', 'staff', 'manager', 'management', 'head', 'director', 'specialist',
  'consultant', 'intern', 'internship', 'associate', 'expert', 'professional', 'officer',
  'graduate', 'trainee', 'coop', 'co', 'op', 'i', 'ii', 'iii', 'iv', 'of', 'the', 'and', 'for',
  'to', 'in', 'a', 'an', 'at', 'with', 'tech', 'technology', 'technical', 'role', 'team', 'global', 'new', 'grad',
]);
// Light synonym canonicalization so ML / AI / "Machine Learning" all match, etc.
const ROLE_SYN = new Map([
  ['ml', 'machinelearning'], ['ai', 'machinelearning'], ['machine', 'machinelearning'], ['learning', 'machinelearning'],
  ['ds', 'datascience'], ['scientist', 'datascience'], ['science', 'datascience'], ['analytics', 'datascience'], ['analytical', 'datascience'],
  ['quant', 'quantitative'], ['mlops', 'dataops'],
]);
const canonRoleTokens = (text) =>
  String(text || '').toLowerCase().replace(/[^a-z0-9+#/ ]/g, ' ').split(/[\s/]+/)
    .map((t) => t.trim()).filter((t) => t.length >= 2 && !GENERIC_ROLE.has(t))
    .map((t) => ROLE_SYN.get(t) || t);

// Build the candidate's on-target term set from profile target_roles + archetypes.
export function targetTerms(targetRoles = [], archetypes = []) {
  const terms = new Set();
  for (const r of targetRoles || []) for (const t of canonRoleTokens(r)) terms.add(t);
  for (const a of archetypes || []) for (const t of canonRoleTokens(typeof a === 'string' ? a : (a && a.name))) terms.add(t);
  return terms;
}
// Does a JD's role TITLE fall within the candidate's target function? Returns
// { fit, onTarget }. NEUTRAL (1.0) when there's no profile or the title carries no
// distinctive token (don't damp what we can't classify).
const OFF_TARGET_FIT = 0.5;
export function functionFit(jdTitle, terms) {
  if (!terms || !terms.size) return { fit: 1, onTarget: true };
  const toks = canonRoleTokens(jdTitle);
  if (!toks.length) return { fit: 1, onTarget: true };
  for (const t of toks) if (terms.has(t)) return { fit: 1, onTarget: true };
  return { fit: OFF_TARGET_FIT, onTarget: false };
}
...
export function jdRequiredYears(jdText, title = '') {
  const t = deEscape(jdText); // "8\+ years" → "8+ years" so the bar is actually found
  const bars = [];
...
export function selfTest() {
  ...
  // Indeed stores "8+ years" as "8\+ years" — de-escape so the requirement is found (the WAVE-Manager bug)
  eq(jdRequiredYears('Minimum 8\\+ years of experience in data science.').years, 8, 'years: de-escapes Indeed "8\\+ years"');
  eq(deEscape('8\\+ years, service\\-oriented, C\\+\\+'), '8+ years, service-oriented, C++', 'deEscape strips markdown/Indeed backslashes');

  // managerial / senior-leadership detection
  ok(isManagerialJd('Manager, Data Science', 'Lead and manage a team of scientists.'), 'manager title → managerial');
  ok(isManagerialJd('Data Scientist', '8+ years, with 3+ years in a people management role'), 'people-management text → managerial');
  ok(!isManagerialJd('Senior Data Scientist', 'Build ML models in Python.'), 'senior IC role is NOT managerial');
  ok(isManagerialJd('', 'Provide proven leadership of large teams.'), 'leadership-of-teams duty in body → managerial (title-less)');
  ok(!isManagerialJd('Senior Data Scientist', 'Deep IC role; no people management.'), 'negated "no people management" not managerial');
  ok(!isManagerialJd('Data Scientist', 'You will report to the Director of Data.'), 'referenced "report to the Director" not managerial');
  ok(!isManagerialJd('Data Scientist', 'Collaborate with engineering managers and product managers.'), 'bare manager NOUNS in body not managerial');
  ...
  // function fit: a role's FUNCTION vs the candidate's target roles (not just tools)
  const terms = targetTerms(['Data Scientist', 'Machine Learning Engineer', 'Data Engineer'], [{ name: 'Quantitative / Finance-tech' }]);
  ok(functionFit('Senior Data Scientist', terms).onTarget, 'Data Scientist → on-target');
  ok(functionFit('Applied Scientist, Alexa AI', terms).onTarget, 'Applied Scientist → on-target (scientist≈data science)');
  ok(functionFit('ML Engineer', terms).onTarget, 'ML Engineer → on-target');
  ok(functionFit('Quantitative Researcher', terms).onTarget, 'Quant Researcher → on-target');
  ok(!functionFit('Reliability Engineer', terms).onTarget, 'Reliability Engineer → OFF-target (shares tools, not function)');
  ok(!functionFit('Mechanical Engineer', terms).onTarget, 'Mechanical Engineer → OFF-target');
  ok(functionFit('Manager, Data Science', terms).onTarget, 'DS Manager → on-target function (seniority handled separately)');
  eq(functionFit('Reliability Engineer', terms).fit < 1, true, 'off-target role is damped (<1)');
  eq(functionFit('anything', new Set()).fit, 1, 'no profile/targets → neutral (1.0)');
