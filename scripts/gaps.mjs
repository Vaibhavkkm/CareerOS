...
export function selfTest() {
  ...
  // Drive the live (skill-aware) match engine: it surfaces Kubernetes as the real
  // cross-job gap here. Assert on that engine-stable property rather than on a
  // specific keyword the taxonomy may classify differently.
  ok(r.skills.some((s) => /kubernet/i.test(s.skill)),
    'roadmap surfaces a real cross-job gap (Kubernetes) from the live match engine');
  ok(r.skills.find((s) => /kubernet/i.test(s.skill)).jobs === 2, 'a gap in both jobs is counted twice');
