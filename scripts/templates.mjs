...
export const THEMES = {
  cv: {
    ...
    modern: { file: 'templates/cv.modern.tex.tmpl', label: 'Modern', desc: 'A subtle navy accent on the name + section rules; same single-column body. Good for product/eng/design.', pages: 1 },
...
function main() {
  ...
  if (args.json) console.log(JSON.stringify({ ok: true, kind: args.kind, default: DEFAULT_THEME, themes }, null, 2));
    ...
    console.log('\n  Pick per build:  /cos build-cv <job> --theme <name>   (or set cv.template in data/profile.yml)');
  }
  process.exit(0);
}
